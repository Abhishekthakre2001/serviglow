...
export default function Quotes() {
  ...
  const columns = [
    ...
    { key: "service", label: "Service" },
    {
      ...
      label: "Status",
      render: (value) => {
        const rawStatus = value?.toLowerCase() || "new";
        ...
        return (
          <span
            className={`px-2 py-1 rounded text-xs ${colors[displayStatus] || "bg-gray-100 text-gray-600"
              } capitalize`}
          >
            {displayStatus}
          </span>
  ...
  const fetchQuotes = async () => {
    try {
      ...
      const res = await contactQuotationApi?.getQuotationbycustomerwise();

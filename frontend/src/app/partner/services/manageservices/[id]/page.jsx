...
export default function ManageServices() {
      ...
      try {
        ...
        if (data.success) {
          setForm({
            ...
            description: data.data.about_service || "",   // ✅ FIX
            price: data.data.price || "",
            estimatedTime: data.data.estimated_time || "", // ✅ FIX
            service_category: data.data.category_id || "", // ✅ FIX
            service_subcategory: data.data.sub_category_id || "", // ✅ FIX
          });
    ...
    const fetchSubcategories = async () => {
      try {
        ...
        const { data } = await subcategoryApi.getSubcategories({
          categoryId: form.service_category,
        });
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("serviceImages", form.description)
    ...
    if (
      ...
      !form.price ||
      !form.estimatedTime ||
      (existingImages.length === 0 && serviceImages.length === 0)
    ) {
      const missingFields = [];

      if (!form.title) missingFields.push("Service Title");
      if (!form.subtitle) missingFields.push("Service Sub Title");
      if (isQuillEmpty(form.description)) missingFields.push("Description");
      if (!form.price) missingFields.push("Price");
      if (!form.estimatedTime) missingFields.push("Estimated Time");

      if (existingImages.length === 0 && serviceImages.length === 0) {
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  {serviceId && existingImages.length > 0 && (
